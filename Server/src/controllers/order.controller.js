  ...
  try {
    ...
    if (!selectedAddress) {
      return res.status(400).json({
        success: false,
        message: "Delivery address selection invalid",
      });
    }
    ...
    const orderItems = user.cart.map((cartItem) => {
      ...
      if (!matchedProduct) {
        throw new Error(
          `Product metadata for item reference ${cartItem.product} could not be resolved`,
        );
      }

      const itemSubtotal = matchedProduct.price * cartItem.quantity;
