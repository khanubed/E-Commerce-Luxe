  ...
  return (
    <div className=" bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="max-w-lg w-full bg-white border border-slate-200 rounded-lg shadow-2xl overflow-hidden transition-all duration-500">
        ...
        {/* STEP 2: REASON FORM + TEXTAREA */}
        {step === 'reason' && (
          <div className="p-8 animate-in slide-in-from-right-4">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Feedback</h2>
              <X size={18} className="text-slate-300 cursor-pointer hover:text-slate-900" onClick={() => navigate('/')} />
            </div>
                ...
                {["Price too high", "Found elsewhere", "Other"].map((r) => (
                  <button 
                    ...
                    className={`w-full p-4 text-left border rounded-md text-xs font-black uppercase tracking-widest transition-all flex justify-between items-center ${
                      reason === r ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-100 text-slate-500 hover:border-slate-300'
                    }`}
                  >
                    {r}
              ...
              {reason !== '' && (
                <div className="animate-in fade-in slide-in-from-top-2">
                  <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2 block">
                    {reason === 'Other' ? 'Please specify' : 'Any additional comments? (Optional)'}
                  </label>
                  <textarea 
        ...
        {step === 'done' && (
          <div className="p-12 text-center animate-in zoom-in-95">
            ...
            <h2 className="text-lg font-black text-slate-900 uppercase tracking-tight">Order Dead</h2>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2 mb-8">Items returned to inventory</p>
