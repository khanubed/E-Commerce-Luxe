...
import { getProductsListByIds } from "../../features/products/productApi.js";
import { createOrderApi, saveAddressApi, verifyPaymentApi } from "../../services/orderApi.js";
import { clearCart } from "../../features/cart/cartSlice.js";
import axios from "axios"; // Required for signature validation endpoint fallback
...
const CheckoutPage = () => {
  ...
  const getCheckoutData = async () => {
    ...
    try {
      setIsLoading(true);
      const productIds = rawCart.map((item) => item.product);
      ...
      console.error("Checkout API Hydration Failed:", error);
      toast.error("Error loading order summary items");
    } finally {
      setIsLoading(false);
    }
  };

  // 3. One direct API call to rule them all
  useEffect(() => {
    if (user) {
      getCheckoutData();
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (data.success && !data.isCOD) {
        ...
        const options = {
          key: import.meta.env.VITE_RAZORPAY_KEY_ID, // ⚠️ Insert your public Key ID string here
